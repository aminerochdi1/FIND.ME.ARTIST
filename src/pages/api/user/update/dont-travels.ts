...
export default async function handler(req: any, res: any) {
    try {
        ...
        if (await checkerUtils.missingParameters(token, travels)) {
        ...
        if (success) {
            try {
                ...
                await profile.setDontTravels(travels)

                return res.status(201).json({ message: "travels_updated" });
            } catch (error) {
                console.error(error)
                return res.status(401).json({ message: "travels_updated_failed" });
            }
        }
